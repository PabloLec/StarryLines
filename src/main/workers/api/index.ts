...
const worker: ExportedHandler<Bindings> = {
  async fetch(req, env) {
    ...
    try {
      ...
      await env.StarryLinesTop.put(topID, JSON.stringify(data), {
  ...
  async scheduled(event, env, ctx) {
    ...
    for (const lang of SUPPORTED_LANGUAGES) {
      ...
      await login(env.MONGO_API_KEY)
        .then(async (client) => {
          return client.db("StarryLines").collection<Repository>(lang).find();
        })
        .then((data) => {
          data.forEach(function (v) {
            delete v._id;
          });
          env.StarryLinesTop.put(lang, JSON.stringify(data), {
            expirationTtl: cacheTtl,
          });
        })
        .then(() => {
